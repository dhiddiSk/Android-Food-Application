...
public class DisplayProductsFragment extends Fragment {
    ...
    private AppCompatEditText et_searchproducts;

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.activity_display_products, container, false);
        et_searchproducts = view.findViewById(R.id.et_searchproducts);
        return view;
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        et_searchproducts.addTextChangedListener(new TextWatcher() {
            @Override
            ...
            @Override
            public void afterTextChanged(Editable enteredWord) {
                    if (!et_searchproducts.getText().toString().isEmpty()){
                        recyclerViewProducts.setVisibility(View.VISIBLE);
                        // filterProducts(et_searchproducts.getText().toString());
                    }
            }
        });
    }
    ...
    private void filterProducts(String searchWord) {
        for (Product product : productsList){
            if (searchWord.toLowerCase().matches(product.getProductName().toLowerCase())){
                productsList.add(product);
                productsListAdapter.notifyDataSetChanged();
            }else{
                Toast.makeText(getActivity(), "No Product Found for word "+searchWord, Toast.LENGTH_SHORT).show();
            }
        }
    }
